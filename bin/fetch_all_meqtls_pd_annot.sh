...
select meta.snp_id,meta.gene as probe,meta.zscore,meta.pvalue,pdsnps.beta as pd_beta,pdsnps.se as pd_beta_se,pdsnps.pvalue,pdsnps.gene from meta_analysis as meta inner join pd_snps_nall as  pdsnps on (meta.snp_id=pdsnps.snpid) where qtl_type='cis' order by meta.pvalue;
END
