...
create table  pd_snps_nall  (snpid  varchar(255), se float, beta float, effect_allele varchar(10), other_allele varchar(10), eaf float, phenotype text,chrom varchar(30), position int unsigned, samplesize mediumint unsigned, ncase mediumint unsigned, ncontrols mediumint unsigned, pvalue float, units text, gene text,  primary key ( snp_id ));
