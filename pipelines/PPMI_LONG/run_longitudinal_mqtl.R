...
# ============================================================
# FULL-SCALE: longitudinal mixed-model mQTL on meta-analysis
# FDR-significant pairs. Cases (~18,986 pairs, PD) and
# Controls (~14,282 pairs, HC), run separately.
#
# Prereq: run prep_full.sh first to generate
#   {group}_geno_subset_full.tsv and {group}_beta_subset_full.csv
# ============================================================
...
# ---- Shared covariate data (loaded once) ----

age_at_visit <- read_csv("../../Downloads/PPMI_covariates/Age_at_visit_14Dec2025.csv",
...
cell_props <- read_csv("../../results/PPMI_GCP/local_results/final/cell_type_proportions.csv",
                        col_types = cols(sample_col_name = col_character()),
                        guess_max = Inf)

participant_status <- read_csv("../../Downloads/PPMI_covariates/Participant_Status_14Dec2025.csv",
                                col_types = cols(PATNO = col_character()),
                                guess_max = Inf)
...
run_longitudinal_group <- function(group, cohort_label, mc_cores = 8) {
  ...
  hits <- read_csv(sprintf("../../results/META_MQTL/meta/%s_sig_fdr05.csv", group),
  ...
  beta_subset <- read_csv(sprintf("../../results/PPMI_LONG/%s_beta_subset_full.csv", group),
  ...
  geno_subset <- read_tsv(sprintf("../../results/PPMI_LONG/%s_geno_subset_full.tsv", group),
  ...
  out_file <- sprintf("../../results/PPMI_LONG/longitudinal_mqtl_%s_results.csv", group)
