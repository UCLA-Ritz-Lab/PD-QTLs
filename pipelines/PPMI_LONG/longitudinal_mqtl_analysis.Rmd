...
`mclapply`-parallelized `lmer` calls — 1,223 pairs (cases) and 167 pairs (controls) in
the current run, a few minutes end to end. (Earlier runs, before ancestry PCs entered
the meta-analysis, carried ~18,986 and ~14,282 pairs and took hours.) **The fitting and
...
follow-up. In the current run **no pair is dynamic in either group**, so the colour
aesthetic is single-valued and every point carries the same shade; the legend entry is
retained so the plot stays comparable with earlier runs, which had 2 dynamic pairs in
cases and 12 in controls.
