...
def main():
    ...
    with open(args.log, "w") as logfile:
        ...
        for bf in beta_files:
            ...
            log(f"  {batch_id}: {len(probes):,} probes", logfile)

        log(f"Non-empty batches: {len(probe_sets)} / {len(beta_files)}",
            logfile)
        if empty_batches:
        ...
        # Intersection of probes across non-empty batches only
        common_probes = sorted(set.intersection(*probe_sets))
        ...
        # Allocate memory-mapped array: probes x samples, float32 saves RAM
        memmap_path = os.path.join(
            os.path.dirname(args.out_matrix), "beta_memmap.dat"
        )
        log(f"Allocating memmap at {memmap_path} "
        ...
        mm[:] = np.nan   # initialise all to NaN
        ...
        mm.flush()
        log("Memmap written and flushed.", logfile)

        # Build combined DataFrame from memmap for downstream steps
        log("Building final DataFrame from memmap...", logfile)
        ...
        if os.path.exists(memmap_path):
            ...
            log("Memmap temp file removed.", logfile)
