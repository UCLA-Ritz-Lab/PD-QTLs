...
# Normalise column names: strip leading # from first column
colnames(psam_raw)[1] <- sub("^#", "", colnames(psam_raw)[1])

# Handle both psam formats:
#   Single-col (#IID):      old WGS pfiles — FID assigned as 0 by PLINK2
#   Two-col (#FID + IID):   imputed pfiles — FID=IID=full sample ID
# In both cases we only need IID for sample matching.
if ("IID" %in% colnames(psam_raw)) {
  psam <- data.frame(IID=psam_raw$IID, stringsAsFactors=FALSE)
} else {
  # Single-column psam: first col is IID
  psam <- data.frame(IID=psam_raw[[1]], stringsAsFactors=FALSE)
}
...
save_dataset <- function(cohort_val, dataset_name, label) {
  ...
  # Standardise ENROLL_AGE to z-score — age ranges ~33-83 while cell type
  # proportions are in [0,1]. Without standardisation the scale difference
  # causes numerical instability in MatrixEQTL producing |beta| >> 1.
  age_mean <- mean(covar_sub_df$ENROLL_AGE, na.rm=TRUE)
  age_sd   <- sd(covar_sub_df$ENROLL_AGE,   na.rm=TRUE)
  covar_sub_df <- covar_sub_df %>%
    mutate(ENROLL_AGE = (ENROLL_AGE - age_mean) / age_sd)
  ...
  write.table(data.frame(FID=stratum$plink_iid, IID=stratum$plink_iid),
              gf, sep=" ", row.names=FALSE, col.names=FALSE, quote=FALSE)
