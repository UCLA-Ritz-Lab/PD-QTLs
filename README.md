...
Genotype imputation (`PEG_WGS`, `PPMI_WGS`) is split into `pre_imputed`
(prepares raw VCFs for upload) and `post_imputed` (processes the returned
dosages), with the Michigan Imputation Server's output landing in
`results/<cohort>_WGS/imputation_results/` rather than `Downloads/` — it's
generated by an external service, not raw input collected directly.
