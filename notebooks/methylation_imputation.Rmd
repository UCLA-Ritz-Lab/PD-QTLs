...
for (chrom in chroms){
  if (is_local || chrom==selected_chr){
    ...
    #merged_all<-filter_cpgs(raw_epic_450k,'EPIC',pheno_list)
    ...
    for (rank in ranks){
      for (lambda in lambdas){
        if (is_local || (rank==selected_rank && lambda==selected_lambda)){
          ...
          #delta_matrix<-mapply(get_deltas,cpgmatrix_is_blanked_peg,merged_450k[,c(peg1_indices,peg2_indices)],completed_450k[,c(peg1_indices,peg2_indices)])
          
          
          completed<-svd_impute(merged,rank,lambda)
